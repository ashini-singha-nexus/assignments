...
def _read_tasks(path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # If the JSON is invalid but the file exists, treat it as empty
        if "Expecting value" in str(e):
            return []
        else:
            # Re-raise for actual JSON syntax errors
            raise
...
def list_tasks(
    ...
    if tags:
        filtered_tasks = [
            task for task in filtered_tasks if any(tag in task["tags"] for tag in tags)
        ]
